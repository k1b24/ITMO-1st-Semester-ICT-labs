...
while len(xml_string) != 0: #идем по строке с xml кодом и постепенно ее уменьшаем
    ...
    if xml_string[:2] == '</': #обработка строки если в ней встретился закрывающий тег
        nexttag = xml_string[3:]
        nexttag = nexttag[nexttag.find('<')+1:]
        nexttag = nexttag[:nexttag.find('>')] # находим следующий тег
        tabs -= 1
        ...
        if nexttag[0] != '/' and xml_string.count('>') > 1: #если следующий тег открывающийся
            json_lines[len(json_lines)-1] += ',' 
        prev_is_content = False     
        r = xml_string.find('>')
        xml_string = xml_string[r+1:]

    elif xml_string[0] == '<': #обработка строки если в ней встретился открывающий тег
        r = xml_string.find('>')
        tag = xml_string[1:r]
